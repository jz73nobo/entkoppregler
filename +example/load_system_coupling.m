function [system, system_properties] = load_system_coupling(sys_string)
	%% init
	E_cell =		{};
	A_cell =		{}; %#ok<NASGU>
	B_cell =		{}; %#ok<NASGU>
...
% 				params(:, 7:9) = params(:, 7:9)./2;
		...
		case 'ABS'
			A_handle = @(p11,p12,p21,p22)[
				0				1			0			0;
				-p11*p12	-p11-p12		0			 0;
				0				0			0			 1;
				0				0	   -p21*p22		-p21-p22
			];
			B_handle = @(p11,p12,p21,p22,k12,k22,K)[
				0						0;
				p11*p12*K*k12			0;
				0						0;
				0				p21*p22*K*k22];
			...
			% Festlegung der Multimodelle
			length_int = [1 1 1 1 5 5];
			int_p11 = linspace(27, 28,		length_int(1));
			int_p12 = linspace(27.5, 27.5,	length_int(2));
			int_p21 = linspace(27.5, 27.5,	length_int(3));
			int_p22 = linspace(27.5, 27.5,	length_int(4));
			int_k12 = linspace(0.8, 1.1,	length_int(5));
			int_k22 = linspace(0.8, 1.1,	length_int(6));

			param_cell = {int_p11,int_p12,int_p21,int_p22,int_k12,int_k22};
			no_params = length(param_cell);
			% calculate all permutations
			cnt_line = 1;
			params = zeros(prod(length_int),no_params);
			for cnt_1 = 1:length_int(1)
				for cnt_2 = 1:length_int(2)
					for cnt_3 = 1:length_int(3)
						for cnt_4 = 1:length_int(4)
							for cnt_5 = 1:length_int(5)
								for cnt_6 = 1:length_int(6)
									params(cnt_line,:) = [
										int_p11(cnt_1), int_p12(cnt_2), int_p21(cnt_3), int_p22(cnt_4), int_k12(cnt_5), int_k22(cnt_6)
									];
									cnt_line = cnt_line + 1;
								end
							end
			...
			R_0 = [
				2.4532 8.2314  3.0271 8.7529;
				17.7354 6.0320 17.1615 5.5104
			];
			F_0 = [
				1, 25;
				1, 1
			...
			for ii = 1:number_models
				A_cell{ii, 1} = A_handle(params(ii, 1), params(ii, 2), params(ii,3), params(ii,4));
				B_cell{ii, 1} = B_handle(params(ii, 1), params(ii, 2), params(ii,3), params(ii,4), params(ii,5), params(ii,6), K);
				C_ref_cell{ii, 1} = C_ref;
			end
