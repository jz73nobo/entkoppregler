...
K_0 = loaded_system_information.RKF_0{2};
F_0 = loaded_system_information.RKF_0{3};
R_0(R_0 == 0) = 1;
F_0(F_0 == 0) = 1;
if true
calculate_hessian = false;
...
switch derivativetype
	case 'R'
		...
		gainval = R_0;
	case 'K'
		error('control:design:gamma:test', 'Requested to show derivative gain but no derivative measurement available for coupling controller design.');
	case 'F'
		indices = indices_F;
		gainval = F_0;
		if ~usereferences
...
for jj = 1:size(indices, 1)
	...
	switch derivativetype
		case 'R'
			R = zeros(size(system_coupling(1).B, 2), size(system_coupling(1).A, 2), size(range, 2));
			for ii = 1:size(range, 2)
				R(:, :, ii) = R_0;
				R(index(1), index(2), ii) = range(ii);
			end
			x_0 = {R, K_0, F_0};
		case 'K'
			K = zeros(size(system_coupling(1).B, 2), size(system_coupling(1).C_dot, 1), size(range, 2));
			for ii = 1:size(range, 2)
				K(:, :, ii) = K_0;
				K(index(1), index(2), ii) = range(ii);
			end
			x_0 = {R_0, K, F_0};
		case 'F'
			F = zeros(size(system_coupling(1).B, 2), size(system_coupling(1).C_ref, 1), size(range, 2));
			for ii = 1:size(range, 2)
				F(:, :, ii) = F_0;
				F(index(1), index(2), ii) = range(ii);
			end
			x_0 = {R_0, K_0, F};
		otherwise
			R = zeros(size(system_coupling(1).B, 2), size(system_coupling(1).A, 1), size(range, 2));
			for ii = 1:size(range, 2)
				R(:, :, ii) = R_0;
				R(index(1), index(2), ii) = range(ii);
			end
	...
	if calculate_hessian
		[c, ceq, gradc, gradceq, hessc, hessceq] = control.design.gamma.ceval(x_0, system_coupling(1), polearea(:, 1:2), weight, opt, derivativetype);
	else
		[c, ceq, gradc, gradceq] = control.design.gamma.ceval(x_0, system_coupling(1), polearea(:, 1:2), weight, opt, derivativetype);
	end
	figure(objectivefigure);
	subplot(max(indices(:, 1)), max(indices(:, 2)), jj);
	plot(range, ceq(cindex, :));
	xlabel(sprintf('$%s(%d, %d)$', lower(derivativetype), index(1), index(2)));
	ylabel(['$c^=_', num2str(cindex), '$']);
	grid on;
	figure(gradientfigure);
	subplot(max(indices(:, 1)), max(indices(:, 2)), jj);
	plot(range, squeeze(gradceq(cindex, index(1), index(2), :)), '-', range(1:end - 1), diff(ceq(cindex, :))./diff(range), ':');
	grid on;
	xlabel(sprintf('$%s(%d, %d)$', lower(derivativetype), index(1), index(2)));
	...
	if calculate_hessian
		figure(hessianfigure);
		hessianidx = sub2ind([size(gainval, 1), size(gainval, 2)], index(1), index(2));
		subplot(max(indices(:, 1)), max(indices(:, 2)), jj);
		plot(range, squeeze(hessceq(hessianidx, hessianidx, cindex, :)), '-', range(1:end - 2), diff(diff(ceq(cindex, :))./diff(range))./diff(range(1:end - 1)), ':', range(1:end - 1), diff(squeeze(gradceq(cindex, index(1), index(2), :))')./diff(range), '.-');
		grid on;
		xlabel(sprintf('$%s(%d, %d)$', lower(derivativetype), index(1), index(2)));
