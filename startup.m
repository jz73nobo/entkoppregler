...
hasstatistic = ismember('Statistics Toolbox', installedToolboxes);
hasstatistic = hasstatistic && logical(license('test', 'Statistics_toolbox'));
if ~hasstatistic
	addpath(genpath(realpath(fullfile(path, 'lib', 'compat', 'Statistic Toolbox'))));
end
hasreportgen = ismember('Matlab Report Generator', installedToolboxes);
hasreportgen = hasreportgen && logical(license('test', 'MATLAB_Report_Gen'));
if ~hasreportgen
	addpath(genpath(realpath(fullfile(path, 'lib', 'compat', 'Report Generator'))));
end
%%
clear encodings i ans e startupexist path v installedToolboxes hasstatistic hasreportgen SNOPT_LICENSE SNOPT_oldinterface GUROBI_HOME;
dbstop if error;